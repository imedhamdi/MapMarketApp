...
async function loadUserSpecificDataAfterLogin() {
    ...
    try {
        showGlobalLoader();
        const [favoritesResponse, threadsResponse] = await Promise.all([
            apiFetch('/favorites').catch(err => {
                console.error("Erreur chargement favoris (catch apiFetch):", err);
                return { status: 'error', message: 'Échec du chargement des favoris', favorites: [] }; // Fallback structuré
            }),
            apiFetch('/messages/threads').catch(err => {
                console.error("Erreur chargement threads (catch apiFetch):", err);
                return { status: 'error', message: 'Échec du chargement des discussions', threads: [] }; // Fallback structuré
            })
        ]);

        // --- Traitement des favoris ---
        if (favoritesResponse && favoritesResponse.favorites && Array.isArray(favoritesResponse.favorites)) {
            APP_STATE.favorites = favoritesResponse.favorites.map(item => item._id || item.id);
            // console.log("Favoris chargés (IDs):", APP_STATE.favorites);
        } else {
            if (favoritesResponse && favoritesResponse.status !== 'error') {
                 console.error("Réponse de l'API pour /favorites (dans loadUserSpecific) : format inattendu :", favoritesResponse);
            }
            APP_STATE.favorites = []; 
        }

        // --- Traitement des threads ---
        if (threadsResponse && threadsResponse.threads && Array.isArray(threadsResponse.threads)) {
            APP_STATE.threads = threadsResponse.threads.map(t => ({ ...t, id: t._id || t.id }));
            // console.log("Threads chargés depuis threadsResponse.threads (dans loadUserSpecific):", APP_STATE.threads);
        } else if (Array.isArray(threadsResponse)) { 
             APP_STATE.threads = threadsResponse.map(t => ({ ...t, id: t._id || t.id }));
             // console.log("Threads chargés directement depuis threadsResponse (tableau, dans loadUserSpecific):", APP_STATE.threads);
        } else {
            if (threadsResponse && threadsResponse.status !== 'error') {
                console.error("Réponse de l'API pour /messages/threads (dans loadUserSpecific) : format inattendu :", threadsResponse);
            }
            APP_STATE.threads = []; 
        }
        ...
        console.error("Erreur majeure lors du chargement des données utilisateur (catch principal de loadUserSpecificDataAfterLogin):", error);
        APP_STATE.favorites = []; 
        APP_STATE.threads = []; 
        updateAuthStateUI(); 
    } finally {
...
// main.js - Correction pour addMarkerToMap
function addMarkerToMap(item) {
    // Vérifier si item.location et item.location.coordinates existent et sont valides
    if (!item.location || !item.location.coordinates || !Array.isArray(item.location.coordinates) || item.location.coordinates.length !== 2) {
        console.warn(`Coordonnées invalides ou manquantes pour l'item: ${item.title || item.id}`, item);
        return;
    }
    ...
    if (typeof latitude !== 'number' || typeof longitude !== 'number') {
        console.warn(`Latitude ou longitude non numérique pour l'item: ${item.title || item.id}`, item);
        return;
    }

    // Leaflet attend [latitude, longitude]
    const marker = L.marker([latitude, longitude], { icon: createCustomMarkerIcon(item) });
    marker.on('click', () => { showItemDetailModal(item.id || item._id); });
    
    if (APP_STATE.markerClusterGroup) {
        APP_STATE.markerClusterGroup.addLayer(marker);
        // console.log(`Marqueur ajouté pour: ${item.title} aux coords [${latitude}, ${longitude}]`);
    } else {
...
async function loadItemsAndRefreshMap() {
    try {
        const responseData = await apiFetch('/items');
        if (responseData && responseData.items && Array.isArray(responseData.items)) {
            APP_STATE.items = responseData.items.map(item => ({ ...item, id: item._id || item.id }));
            refreshMapMarkers();
        } else {
            console.error("La réponse de l'API pour /items n'a pas le format attendu :", responseData);
            APP_STATE.items = [];
            refreshMapMarkers();
        }
    } catch (error) {
...
// main.js - Fonction showItemDetailModal CORRIGÉE

function showItemDetailModal(itemId) {
    const item = APP_STATE.items.find(i => (i.id || i._id) === itemId);
    if (!item) {
        console.error("Article non trouvé pour affichage détail:", itemId);
        return;
    }

    // console.log("Affichage détail pour item:", JSON.stringify(item, null, 2)); // Pour débogage
    ...
    // --- Correction pour l'affichage des coordonnées et de la distance ---
    const locationTextEl = document.getElementById('itemDetailLocationText');
    const distanceEl = document.getElementById('itemDetailDistance');
    let itemLatitude, itemLongitude; // Déclarer les variables pour les utiliser plus tard

    if (item.location && item.location.coordinates && Array.isArray(item.location.coordinates) && item.location.coordinates.length === 2) {
        itemLongitude = item.location.coordinates[0];
        itemLatitude = item.location.coordinates[1];

        if (typeof itemLatitude === 'number' && typeof itemLongitude === 'number') {
            locationTextEl.textContent = `Proche de (${itemLatitude.toFixed(2)}, ${itemLongitude.toFixed(2)})`; // Affichage standard [lat, lng]

            if (APP_STATE.userLocation) { // Vérifier si APP_STATE.userLocation est défini
                const dist = haversineDistance(APP_STATE.userLocation, { lat: itemLatitude, lng: itemLongitude });
            ...
            locationTextEl.textContent = "Coordonnées invalides";
            distanceEl.classList.add('hidden');
        }
    } else {
        locationTextEl.textContent = "Localisation non disponible";
        distanceEl.classList.add('hidden');
    }
    // --- Fin de la correction pour les coordonnées et la distance ---
    ...
    // Correction pour les informations du vendeur :
    // Supposons que item.userId est l'objet utilisateur populé par le backend.
    const sellerInfo = item.userId;
    if (sellerInfo && typeof sellerInfo === 'object' && sellerInfo.username) { // Vérifier que sellerInfo est bien un objet avec username
        document.getElementById('itemDetailSellerAvatar').src = sellerInfo.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(sellerInfo.username)}&background=A78BFA&color=fff&size=48`;
        document.getElementById('itemDetailSellerName').textContent = sellerInfo.username;
        const ratingStarsContainer = document.getElementById('itemDetailSellerRating');
        ratingStarsContainer.innerHTML = '';
        const score = sellerInfo.rating?.score || 0;
        const count = sellerInfo.rating?.count || 0;
        for (let i = 1; i <= 5; i++) {
        ...
        // Fallback si item.userId n'est pas l'objet attendu
        console.warn("Informations du vendeur (item.userId) non disponibles ou format incorrect pour l'item:", item);
        document.getElementById('itemDetailSellerName').textContent = "Vendeur";
    ...
    // Assurez-vous que toggleFavorite est bien défini et gère l'état correctement
    ...
    // Assurez-vous que item.userId (ou sellerInfo._id) est bien l'ID du vendeur
    const sellerId = sellerInfo?._id || sellerInfo?.id || item.userId; // S'assurer d'avoir un ID pour la comparaison

    if (APP_STATE.currentUser && sellerId && APP_STATE.currentUser.id === sellerId) {
        ...
        contactBtn.onclick = () => {
            ...
            if (!sellerId) {
                showToast("Informations du vendeur non disponibles pour démarrer une discussion.", 3000, 'error');
                return;
            }
            closeModal('itemDetailModal');
            startNewOrOpenChat(sellerId, (item.id || item._id), item.title, item.images ? item.images[0] : null);
        };
    ...
    document.getElementById('itemDetailGoToBtn').onclick = () => {
        // Utiliser les variables itemLatitude et itemLongitude extraites et vérifiées
        if (typeof itemLatitude === 'number' && typeof itemLongitude === 'number') {
            window.open(`https://maps.google.com/?q=${itemLatitude},${itemLongitude}`, '_blank');
        } else {
...
async function loadThreads() {
    if (!APP_STATE.currentUser) return;
    try {
        // 'responseDataForThreads' reçoit l'objet complet de l'API
        const responseDataForThreads = await apiFetch('/messages/threads');
        console.log('Réponse de /api/messages/threads (dans loadThreads):', JSON.stringify(responseDataForThreads, null, 2)); // Loggez pour vérifier la structure

        // Vérifiez la structure et extrayez le tableau
        // Adaptez 'responseDataForThreads.threads' si la clé est différente (ex: .data, .items, etc.)
        if (responseDataForThreads && responseDataForThreads.threads && Array.isArray(responseDataForThreads.threads)) {
            APP_STATE.threads = responseDataForThreads.threads.map(t => ({ ...t, id: t._id || t.id }));
        }
        // Cas où l'API pourrait renvoyer directement un tableau (moins probable si vos API sont cohérentes)
        else if (Array.isArray(responseDataForThreads)) {
            APP_STATE.threads = responseDataForThreads.map(t => ({ ...t, id: t._id || t.id }));
        }
        // Si la structure n'est pas celle attendue ou si une erreur s'est produite plus tôt (dans apiFetch)
        else {
            console.error("La réponse de l'API pour /messages/threads (dans loadThreads) n'a pas le format attendu ou est vide :", responseDataForThreads);
            APP_STATE.threads = []; // Initialiser à un tableau vide pour éviter les erreurs .map/.reduce/.sort
        }

        updateAuthStateUI(); // APP_STATE.threads devrait maintenant être un tableau (éventuellement vide)
        if (APP_STATE.currentOpenModal?.id === 'messagingModal' && !document.getElementById('threadsListView').classList.contains('hidden')) {
            renderThreadsList();
        }
    } catch (error) {
        // Ce catch est atteint si apiFetch rejette une promesse (erreur réseau, statut non-ok qui n'a pas été transformé en structure attendue par le .catch de apiFetch)
        console.error("Erreur chargement threads (catch principal dans loadThreads):", error);
        APP_STATE.threads = []; // S'assurer que c'est un tableau vide en cas d'erreur majeure
        updateAuthStateUI(); // Mettre à jour l'UI pour refléter l'état vide
    }
}
